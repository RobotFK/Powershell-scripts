...
$global:arrayline = $null ;#Current line we are reading out (This need acess so we can skip on multiline readouts)
$global:Currentrow = "Time,PC-Name,Username,PC-SN,USB,Additional Monitors,PastMonitors" #This will be how the data is stored each line,this one is also the top line
...
function Handle-CurrentMonitor {
    ...
    #$temp = $output.count;Write-Host "Line $temp has the monitors: $monitors"
...
<#
    ...
    Add-content -path R:\Inventurdaten\Output.txt -value $output;
...
#$dataarray =  Get-Content -Path R:\Inventurdaten\R9121F6G-7-2.txt;
#$dataarray += "$([char]30)" #This is imperative
#$dataarray += Get-Content -Path R:\Inventurdaten\R91234KA-7-2.txt;
$dataarray += "$([char]30)"
...
Write-Progress -Id 6 -Activity "File Progress" -Status "($stage/8*100)% Complete:" -PercentComplete ($stage/8*100)
